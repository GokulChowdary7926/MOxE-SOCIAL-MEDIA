...
export default function PostMenu({ postId, authorId, onPostDeleted, onPostArchived, onPostHidden }: PostMenuProps) {
  ...
  const handleUnfollow = async () => {
    if (!confirm(`Unfollow this user? You won't see their posts anymore.`)) {
      return
    }
  ...
  const handleBlock = async () => {
    if (!confirm(`Block this user? You won't see their posts or be able to message them.`)) {
      return
    }
  ...
  const handleRemoveFromGrid = async () => {
    try {
      await api.post(`/posts/${postId}/hide-from-profile`)
      setIsOpen(false)
      alert('Post removed from main grid')
    } catch (error) {
  ...
  const handlePinToGrid = async () => {
    try {
      ...
      setIsOpen(false)
      alert('Post pinned to main grid')
    } catch (error) {
  ...
  const handleToggleComments = async () => {
    try {
      await api.post(`/posts/${postId}/toggle-comments`)
      setIsOpen(false)
  ...
  const handleAdjustPreview = () => {
    setIsOpen(false)
    navigate(`/edit-post/${postId}?adjust=true`)
  }
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="w-8 h-8 rounded-full hover:bg-light-gray/20 active:bg-light-gray/30 flex items-center justify-center transition-colors touch-manipulation"
      ...
      {isOpen && (
        <>
          ...
          {/* Menu - Bottom Sheet for Own Post, Dropdown for Others */}
          {isOwnPost ? (
            // OWN POST MENU - Bottom Sheet Style
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-medium-gray rounded-t-3xl shadow-2xl max-h-[85vh] overflow-y-auto">
              ...
              <div className="px-4 pb-4 border-b border-light-gray/20">
                <h3 className="text-white font-semibold text-base">Posts</h3>
